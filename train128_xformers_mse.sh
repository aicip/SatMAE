...
python3 main_pretrain.py --use-xformers --train_path "$IN_PATH" --output_dir "$OUT_DIR" --model="$MODEL" --input_size "$INPUT_SIZE" --patch_size "$PATCH_SIZE" --batch_size "$BATCH_SIZE" --epochs "$EPOCHS" --loss "$LOSS" --attn_name "$ATTENTION" --ffn_name="$FFN_NAME" $@
